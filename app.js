/**
 * =================================================================
 * SCRIPT UTAMA FRONTEND - JURNAL PEMBELAJARAN (VERSI DENGAN LOGIKA UI REKAP)
 * =================================================================
 * @version 5.6 - Perbaikan Logika Tampilan Rekap Nilai
 * @author Gemini AI Expert for User
 *
 * FITUR UTAMA VERSI INI:
 * - [PERBAIKAN] Spinner loading pada halaman Rekap Nilai kini hanya muncul saat proses fetch data.
 * - [UX] Area hasil rekapitulasi akan otomatis disembunyikan setiap kali filter diubah,
 *   mencegah tampilan data yang tidak relevan.
 */
...
const SCRIPT_URL = "https://script.google.com/macros/s/AKfycbwY8QlkIhKPtXpUykJvir6JhDhRqAyTvVaQFfAHR7l7YFsIiHI8emI3_SwvgT-E1J3Vlw/exec";

let cachedSiswaData = [], cachedJurnalHistory = [], cachedUsers = [], cachedJenisNilai = [], relationalFilterData = [];
...
function onRekapTahunAjaranChange() { sembunyikanAreaRekap(); const selectedTahun = rekapFilterTahunAjaranEl.value; resetAndDisableDropdown(rekapFilterSemesterEl, '-- Pilih Semester --'); resetAndDisableDropdown(rekapFilterKelasEl, '-- Pilih Kelas --'); resetAndDisableDropdown(rekapFilterMataPelajaranEl, '-- Pilih Mapel --'); if (!selectedTahun) return; const availableSemesters = [...new Set(relationalFilterData.filter(item => item.tahunAjaran == selectedTahun).map(item => item.semester).filter(Boolean))].sort(); populateDropdown('rekapFilterSemester', availableSemesters, '-- Pilih Semester --'); rekapFilterSemesterEl.disabled = false; }
function onRekapSemesterChange() { sembunyikanAreaRekap(); const selectedSemester = rekapFilterSemesterEl.value; resetAndDisableDropdown(rekapFilterKelasEl, '-- Pilih Kelas --'); resetAndDisableDropdown(rekapFilterMataPelajaranEl, '-- Pilih Mapel --'); if (!selectedSemester) return; const availableKelas = [...new Set(relationalFilterData.filter(item => item.tahunAjaran == rekapFilterTahunAjaranEl.value && item.semester == selectedSemester).map(item => item.kelas).filter(Boolean))].sort(); populateDropdown('rekapFilterKelas', availableKelas, '-- Pilih Kelas --'); rekapFilterKelasEl.disabled = false; }
function onRekapKelasChange() { sembunyikanAreaRekap(); const selectedKelas = rekapFilterKelasEl.value; resetAndDisableDropdown(rekapFilterMataPelajaranEl, '-- Pilih Mapel --'); if (!selectedKelas) return; const availableMapel = [...new Set(relationalFilterData.filter(item => item.tahunAjaran == rekapFilterTahunAjaranEl.value && item.semester == rekapFilterSemesterEl.value && item.kelas == selectedKelas).flatMap(item => item.mapel).filter(Boolean))].sort(); populateDropdown('rekapFilterMataPelajaran', availableMapel, '-- Pilih Mapel --'); rekapFilterMataPelajaranEl.disabled = false; }
...
// --- 3.8. REKAP NILAI ---
function sembunyikanAreaRekap() { if (areaRekapNilai) { areaRekapNilai.classList.add('hidden'); } }
async function tampilkanRekapNilai() {
    const filter = { tahunAjaran: rekapFilterTahunAjaranEl.value, semester: rekapFilterSemesterEl.value, kelas: rekapFilterKelasEl.value, mapel: rekapFilterMataPelajaranEl.value, };
...
function setupDashboardListeners() {
    ...
    navButtons.forEach(button => {
        button.addEventListener('click', () => {
            ...
            if (sectionId === 'riwayatSection') { initHistoryCascadingFilters(); document.getElementById('riwayatTableBody').innerHTML = '<tr><td colspan="7" style="text-align: center;">Gunakan filter di atas untuk menampilkan riwayat.</td></tr>'; document.getElementById('exportRiwayatButton').style.display = 'none'; } 
            else if(sectionId === 'nilaiSection') { initNilaiCascadingFilters(); areaInputNilai.classList.add('hidden'); } 
            else if(sectionId === 'rekapSection') { initRekapCascadingFilters(); sembunyikanAreaRekap(); } 
    ...
    rekapFilterMataPelajaranEl?.addEventListener('change', sembunyikanAreaRekap);
