 ...
 * =================================================================
 * @version 5.5 - Implementasi Token Sesi & Refactoring
 ...
 * - [UX] Penanganan error pada form dipastikan tidak menghapus input pengguna.
 */
...
const SCRIPT_URL = "https://script.google.com/macros/s/AKfycbw_Qy09spaFFEPKZkMLVOksuGcBmtpE2EK4CQdFUTU9c_1ytoww_u3Bj6wQWlFWvCQI_w/exec";
...
/**
 * --- BARU: Fungsi helper untuk menambahkan token sesi ke parameter URL ---
 * @returns {string} Bagian URL yang berisi parameter authToken.
 */
...
/**
 * --- BARU: Fungsi helper untuk menambahkan token sesi ke objek FormData ---
 * @param {FormData} formData - Objek FormData yang akan ditambahkan token.
 * @returns {FormData} Objek FormData yang sudah berisi token.
 */
...
function checkAuthentication() {
    ...
    if (!user) {
        ...
        if (userData.peran && userData.peran.toLowerCase() !== 'admin') {
            ...
            document.querySelector('button[data-section="siswaSection"]')?.style.display = 'none';
        }
    }
}
...
async function handleLogin() {
    ...
    try {
        ...
        if (result.status === "success") {
            ...
            sessionStorage.setItem('authToken', result.data.token); // Simpan token sesi
...
function handleLogout() {
    if (confirm('Apakah Anda yakin ingin logout?')) {
        ...
        sessionStorage.removeItem('authToken'); // Hapus token sesi
        window.location.href = 'index.html';
    }
}
...
async function preloadAllData() {
    ...
    const authToken = getAuthTokenParam();

    const dataPromises = [
        fetch(`${SCRIPT_URL}?action=getJurnalHistory${authToken}`).then(res => res.json()),
        fetch(`${SCRIPT_URL}?action=getUniqueJenisNilai${authToken}`).then(res => res.json())
    ];
    if (isAdmin) {
        dataPromises.push(
            fetch(`${SCRIPT_URL}?action=searchSiswa&searchTerm=${authToken}`).then(res => res.json()),
            fetch(`${SCRIPT_URL}?action=getUsers${authToken}`).then(res => res.json())
        );
    }
    try {
...
async function initCascadingFilters() {
    ...
    try {
        const response = await fetch(`${SCRIPT_URL}?action=getRelationalFilterData${getAuthTokenParam()}`);
        const result = await response.json();
        if (result.status === 'success') {
            ...
            showStatusMessage('Gagal memuat data filter utama: ' + result.message, 'error');
        }
    } catch (error) {
...
// --- 3.3. MANAJEMEN SISWA ---
async function refreshSiswaCache() { showLoading(true); try { const response = await fetch(`${SCRIPT_URL}?action=searchSiswa&searchTerm=${getAuthTokenParam()}`); const result = await response.json(); if (result.status === 'success') { cachedSiswaData = result.data; showStatusMessage('Data siswa berhasil diperbarui.', 'success'); } else { showStatusMessage('Gagal memperbarui data siswa: ' + result.message, 'error'); } } catch (error) { showStatusMessage('Kesalahan jaringan saat memperbarui data siswa.', 'error'); } finally { showLoading(false); } }
...
async function saveSiswa() { const form = document.getElementById('formSiswa'); let formData = new FormData(form); const oldNisn = document.getElementById('formNisnOld').value; const action = oldNisn ? 'updateSiswa' : 'addSiswa'; formData.append('action', action); if (oldNisn) formData.append('oldNisn', oldNisn); formData = addAuthTokenToFormData(formData); showLoading(true); try { const response = await fetch(SCRIPT_URL, { method: 'POST', body: formData }); const result = await response.json(); if (result.status === 'success') { showStatusMessage(result.message, 'success'); resetFormSiswa(); await refreshSiswaCache(); searchSiswa(); hasLoadedRelationalData = false; initCascadingFilters(); initNilaiCascadingFilters(); } else { showStatusMessage(`Gagal: ${result.message}`, 'error'); } } catch (error) { showStatusMessage(`Terjadi kesalahan jaringan: ${error.message}`, 'error'); } finally { showLoading(false); } }
function editSiswaHandler(nisn) { const siswa = cachedSiswaData.find(s => s.NISN == nisn); if (!siswa) return; document.getElementById('formNisn').value = siswa.NISN; document.getElementById('formNama').value = siswa.Nama; document.getElementById('formKelas').value = siswa.Kelas; document.getElementById('formTahunAjaran').value = siswa.TahunAjaran; document.getElementById('formMapel').value = siswa.MataPelajaran || ''; document.getElementById('formSemesterSiswa').value = siswa.Semester || ''; document.getElementById('formNisnOld').value = siswa.NISN; const saveButton = document.getElementById('saveSiswaButton'); saveButton.textContent = 'Update Data Siswa'; saveButton.classList.add('btn-primary'); document.getElementById('formSiswa').scrollIntoView({ behavior: 'smooth' }); }
function resetFormSiswa() { document.getElementById('formSiswa').reset(); document.getElementById('formNisnOld').value = ''; const saveButton = document.getElementById('saveSiswaButton'); saveButton.textContent = 'Simpan Data Siswa'; saveButton.classList.remove('btn-primary'); }
async function deleteSiswaHandler(nisn) { if (confirm(`Apakah Anda yakin ingin menghapus siswa dengan NISN: ${nisn}?`)) { showLoading(true); let formData = new FormData(); formData.append('action', 'deleteSiswa'); formData.append('nisn', nisn); formData = addAuthTokenToFormData(formData); try { const response = await fetch(SCRIPT_URL, { method: 'POST', body: formData }); const result = await response.json(); if (result.status === 'success') { showStatusMessage(result.message, 'success'); await refreshSiswaCache(); searchSiswa(); hasLoadedRelationalData = false; initCascadingFilters(); initNilaiCascadingFilters(); } else { showStatusMessage(`Gagal menghapus: ${result.message}`, 'error'); } } catch (error) { showStatusMessage(`Terjadi kesalahan jaringan: ${error.message}`, 'error'); } finally { showLoading(false); } } }
...
/** --- REFACTOR: Fungsi terpusat untuk memfilter riwayat --- */
...
function exportRiwayatToExcel() {
    const dataToExport = getFilteredRiwayatData().map(j => ({ Tanggal: new Date(j.Tanggal).toLocaleDateString('id-ID'), "Tahun Ajaran": j.TahunAjaran, Semester: j.Semester, Kelas: j.Kelas, "Mata Pelajaran": j.MataPelajaran, Materi: j.Materi, Catatan: j.Catatan, Periode: j.Periode, Kehadiran: j.Kehadiran }));
...
async function refreshUserCache() { showLoading(true); try { const response = await fetch(`${SCRIPT_URL}?action=getUsers${getAuthTokenParam()}`); const result = await response.json(); if (result.status === 'success') { cachedUsers = result.data; showStatusMessage('Data pengguna berhasil diperbarui.', 'success'); } else { showStatusMessage('Gagal memperbarui data pengguna: ' + result.message, 'error'); } } catch (error) { showStatusMessage('Kesalahan jaringan saat memuat pengguna.', 'error'); } finally { showLoading(false); } }
...
async function saveUser() { const oldUsername = document.getElementById('formUsernameOld').value; const action = oldUsername ? 'updateUser' : 'addUser'; let formData = new FormData(); formData.append('action', action); formData.append('nama', document.getElementById('formNamaPengguna').value); formData.append('username', document.getElementById('formUsername').value); formData.append('password', document.getElementById('formPassword').value); formData.append('peran', document.getElementById('formPeran').value); if (oldUsername) formData.append('oldUsername', oldUsername); if (action === 'addUser' && !formData.get('password')) return showStatusMessage('Password wajib diisi untuk pengguna baru.', 'error'); formData = addAuthTokenToFormData(formData); showLoading(true); try { const response = await fetch(SCRIPT_URL, { method: 'POST', body: formData }); const result = await response.json(); if (result.status === 'success') { showStatusMessage(result.message, 'success'); resetFormPengguna(); await refreshUserCache(); loadUsers(); } else { showStatusMessage(`Gagal: ${result.message}`, 'error'); } } catch (error) { showStatusMessage(`Terjadi kesalahan jaringan: ${error.message}`, 'error'); } finally { showLoading(false); } }
...
async function deleteUserHandler(username) { const loggedInUser = JSON.parse(sessionStorage.getItem('loggedInUser')); if (loggedInUser && loggedInUser.username === username) return showStatusMessage('Anda tidak dapat menghapus akun Anda sendiri.', 'error'); if (confirm(`Apakah Anda yakin ingin menghapus pengguna '${username}'?`)) { showLoading(true); let formData = new FormData(); formData.append('action', 'deleteUser'); formData.append('username', username); formData = addAuthTokenToFormData(formData); try { const response = await fetch(SCRIPT_URL, { method: 'POST', body: formData }); const result = await response.json(); if (result.status === 'success') { showStatusMessage(result.message, 'success'); await refreshUserCache(); loadUsers(); } else { showStatusMessage(`Gagal: ${result.message}`, 'error'); } } catch (error) { showStatusMessage(`Terjadi kesalahan jaringan: ${error.message}`, 'error'); } finally { showLoading(false); } } }
...
async function loadSiswaUntukNilai() {
    ...
    try {
        const response = await fetch(`${SCRIPT_URL}?${params}${getAuthTokenParam()}`);
        const result = await response.json();
...
async function submitNilai() {
    const detailNilai = { tahunAjaran: nilaiFilterTahunAjaranEl.value, semester: nilaiFilterSemesterEl.value, kelas: nilaiFilterKelasEl.value, mataPelajaran: nilaiFilterMataPelajaranEl.value, jenisNilai: jenisNilaiInput.value.trim() };
    if (!detailNilai.jenisNilai) return showStatusMessage('Harap isi kolom "Jenis Penilaian" terlebih dahulu.', 'error');
    const nilaiSiswa = [];
    document.querySelectorAll('#nilaiTableBody tr').forEach(row => { const nilaiInput = row.querySelector('.nilai-input'); if (nilaiInput && nilaiInput.value !== '') { nilaiSiswa.push({ nisn: row.dataset.nisn, nama: row.dataset.nama, nilai: parseFloat(nilaiInput.value) }); } });
    if (nilaiSiswa.length === 0) return showStatusMessage('Tidak ada nilai yang diisi. Harap masukkan setidaknya satu nilai siswa.', 'error');
    ...
    showLoading(true);
    try {
        const urlWithToken = `${SCRIPT_URL}?action=submitNilai${getAuthTokenParam()}`;
        const response = await fetch(urlWithToken, { method: 'POST', mode: 'cors', headers: { 'Content-Type': 'text/plain;charset=utf-8' }, body: JSON.stringify(dataUntukKirim) });
        const result = await response.json();
        if (result.status === 'success') {
            ...
            nilaiTableHead.innerHTML = '';
            nilaiTableBody.innerHTML = '';
            fetch(`${SCRIPT_URL}?action=getUniqueJenisNilai${getAuthTokenParam()}`).then(res => res.json()).then(result => { if(result.status === 'success') cachedJenisNilai = result.data; });
        } else {
...
function showJenisNilaiSuggestions() {
    ...
    if (query.length === 0) { jenisNilaiSaranEl.classList.add('hidden'); return; }
    const suggestions = cachedJenisNilai.filter(jenis => jenis.toLowerCase().includes(query));
    if (suggestions.length > 0) {
        suggestions.forEach(suggestion => { const itemDiv = document.createElement('div'); itemDiv.className = 'rekomendasi-item'; itemDiv.textContent = suggestion; itemDiv.onclick = () => { jenisNilaiInput.value = suggestion; jenisNilaiSaranEl.classList.add('hidden'); jenisNilaiInput.dispatchEvent(new Event('input')); }; jenisNilaiSaranEl.appendChild(itemDiv); });
...
async function checkExistingNilai() {
    const detailFilter = { tahunAjaran: nilaiFilterTahunAjaranEl.value, semester: nilaiFilterSemesterEl.value, kelas: nilaiFilterKelasEl.value, mapel: nilaiFilterMataPelajaranEl.value, jenisNilai: jenisNilaiInput.value.trim() };
    ...
    try {
        ...
        if (result.status === 'success' && result.data.length > 0) {
            ...
            document.querySelectorAll('#nilaiTableBody tr').forEach(row => { const nisn = row.dataset.nisn; if (nilaiMap.has(nisn)) { const nilaiInput = row.querySelector('.nilai-input'); nilaiInput.value = nilaiMap.get(nisn); } });
        }
    } catch (error) {
...
function setupDashboardListeners() {
    ...
    riwayatTahunAjaranEl?.addEventListener('change', onHistoryTahunAjaranChange);
    riwayatSemesterEl?.addEventListener('change', onHistorySemesterChange);
    riwayatKelasEl?.addEventListener('change', onHistoryKelasChange);
